...
3. **유저웹은 서버 렌더.** `renderContentToHTML(blocks)`은 `ServerBlockNoteEditor`의 **`blocksToHTMLLossy`** 로 **깔끔한 시맨틱 HTML**(`<h2>`, `<aside class="callout">`, `<table>`…)을 만든다 → `generateStaticParams` + `revalidate`로 SSG/ISR. 이게 SEO의 핵심.
4. **편집 = 노출.** 유저웹은 `@repo/editor/content.css`로 렌더 결과를 스타일링하고, 에디터도 같은 CSS를 import 해 커스텀 블록이 동일하게 보인다.
